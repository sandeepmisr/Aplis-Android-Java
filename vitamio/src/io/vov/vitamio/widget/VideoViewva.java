 ...
 */
public class VideoViewva extends SurfaceView implements MediaControllermr.MediaPlayerControl {
  ...
  OnPreparedListener mPreparedListener = new OnPreparedListener() {
    public void onPrepared(MediaPlayer mp) {
      ...
      if (mMediaControllermr != null)
        mMediaControllermr.setEnabled(true);
      mVideoWidth = mp.getVideoWidth();
      mVideoHeight = mp.getVideoHeight();
      mVideoAspectRatio = mp.getVideoAspectRatio();
      ...
      if (mVideoWidth != 0 && mVideoHeight != 0) {
        ...
        if (mSurfaceWidth == mVideoWidth && mSurfaceHeight == mVideoHeight) {
          if (mTargetState == STATE_PLAYING) {
            start();
            if (mMediaControllermr != null)
              mMediaControllermr.show();
          } else if (!isPlaying() && (seekToPosition != 0 || getCurrentPosition() > 0)) {
            if (mMediaControllermr != null)
              mMediaControllermr.show(0);
          }
        }
  ...
  SurfaceHolder.Callback mSHCallback = new SurfaceHolder.Callback() {
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
      ...
      if (mMediaPlayer != null && isValidState && hasValidSize) {
        ...
        start();
        if (mMediaControllermr != null) {
          if (mMediaControllermr.isShowing())
            mMediaControllermr.hide();
          mMediaControllermr.show();
        }
      }
    }
	...
	private MediaControllermr mMediaControllermr;
  ...
  public VideoViewva(Context context) {
    super(context);
    initVideoView(context);
  }

  public VideoViewva(Context context, AttributeSet attrs) {
    this(context, attrs, 0);
    initVideoView(context);
  }

  public VideoViewva(Context context, AttributeSet attrs, int defStyle) {
...
//      return;
  ...
  public void setMediaController(MediaControllermr controller) {
    if (mMediaControllermr != null)
      mMediaControllermr.hide();
    mMediaControllermr = controller;
    attachMediaController();
  }
  ...
  private void attachMediaController() {
    if (mMediaPlayer != null && mMediaControllermr != null) {


      mMediaControllermr.setMediaPlayer(this);
      View anchorView = this.getParent() instanceof View ? (View) this.getParent() : this;
      mMediaControllermr.setAnchorView(anchorView);
      mMediaControllermr.setEnabled(isInPlaybackState());

      if (mUri != null) {
        List<String> paths = mUri.getPathSegments();
        String name = paths == null || paths.isEmpty() ? "null" : paths.get(paths.size() - 1);
        mMediaControllermr.setFileName(name);
      }
    }
  }
  ...
  public boolean onTouchEvent(MotionEvent ev) {
    if (isInPlaybackState() && mMediaControllermr != null)
  ...
  public boolean onKeyDown(int keyCode, KeyEvent event) {
    ...
    if (isInPlaybackState() && isKeyCodeSupported && mMediaControllermr != null) {
